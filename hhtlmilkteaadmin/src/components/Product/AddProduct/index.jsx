...
const AddProduct = () => {
    ...
    const onSubmit = (data) => {
        ...
        data.additionOptions = left;
        data.sizeOptions = leftSize;
    ...
    const handleOpenAdd = () => {
        if (addOpen === 0) {
            setaddOpen(1);
            setRight(additionOptions);
        }
        setOpen(true);
    }

    const handleRemoveAdd = (id) => {
        setLeft(left.filter(e => e.id !== id));
        setRight([...right, additionOptions.find(e => e.id === id)]);
    }
    ...
    const handleOpenAddSize = () => {
        if (addOpenSize === 0) {
            setaddOpenSize(1);
            setRightSize(sizeOptions);
        }
        setOpenSize(true);
    }

    const handleRemoveSize = (id) => {
        setLeftSize(leftSize.filter(e => e.id !== id));
        setRightSize([...rightSize, sizeOptions.find(e => e.id === id)]);
    }
                        ...
                        <Grid container spacing={3}>
                            <Grid item md={8} xs={12}>
                                ...
                                <TextField label="Nhập giá tiền"
                                    ...
                                    fullWidth
                                    name="price"
                                    inputRef={register({ required: "Nhập giá sản phẩm", setValueAs: v => parseInt(v) * 1000 })}
                                />
                                {errors.price?.message &&
                                    <FormHelperText style={{ color: 'red' }} id="component-error-text">Nhập giá sản phẩm</FormHelperText>
                                }
